...
class FirestoreTagsRepository(
    ...
    override suspend fun addTag(tag: Tag): ResultStatus<Boolean> {
    ...
    override suspend fun deleteTag(tag: Tag): ResultStatus<Boolean> {
        if (tag !in tags.value) return ResultStatus.Success(false)
        ...
        val error: ResultStatus.Error<Boolean>? = try {
            ...
            if (tagInUse is ResultStatus.Success && tagInUse.result) {
                ResultStatus.Error(false, ResultStatus.ErrorType.LocalWarning.TagUsedInNote)
            } else {
                null
            }
        } catch (e: Throwable) {
