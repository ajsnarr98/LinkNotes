...
interface NotesRepository {
    ...
    data class PendingChange(
        val action: Action,
        val resultStatus: ResultStatus<Note>,
    )
     ...
     */
    suspend fun arePendingChangesForNote(note: Note): ResultStatus<Boolean>
     ...
     */
    suspend fun saveNote(note: Note): ResultStatus<Note>
     ...
     * If successful, updates [notes] respectively.
     */
    suspend fun deleteNote(note: Note): ResultStatus<Note>

    /**
     * Returns whether or not any notes contain the given tag. Check
     * includes pending changes to notes.
     */
    suspend fun containsTag(tag: Tag): ResultStatus<Boolean>
}
